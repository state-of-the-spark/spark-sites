...
description: Set up, build, launch, or create Google Ads for a small business of ANY type — local and home services (landscaper, cleaner, HVAC, roofer, plumber), brick-and-mortar storefronts, regional or national lead-gen (consultants, agencies, B2B), and online businesses. Trigger on phrasings like "run some ads on Google," "advertise my business," "set up Google Ads," "get more calls or leads," "do some PPC," "AdWords," or "get on Google" — including first-timers on a small budget (≤$10/day) and "build it and leave it paused for me to review." Classifies the business from its website, then drives the user's Chrome browser to build the account end-to-end (structure, keywords, negatives, RSAs, geo-targeting, dayparting, conversion tracking), adapting each step to the business profile. Do NOT use for Maps-only listings with no ad spend, optimize-only requests on existing campaigns, or Shopping-feed builds (Search campaigns for online sellers are fine; Shopping feeds are a future skill).
---
